...
non_leaf_aboveground_litterfall_new = xr.DataArray(
    np.full((365,17,112,192), -99999.0),
    dims=('time','PFT', 'lat', 'lon'),
    coords={'time': ds['time'],'PFT': ds['PFT'], 'lat': ds['lat'], 'lon': ds['lon']},
...
Belowground_litter_fall_new = xr.DataArray(
    np.full((365,17,112,192), -99999.0),
    dims=('time','PFT', 'lat', 'lon'),
    coords={'time': ds['time'],'PFT': ds['PFT'], 'lat': ds['lat'], 'lon': ds['lon']},
)
...
for day in range(0,365):
    print(day)
    Leaf_fall_tmp=dataset['Leaf_fall'][day,:]
    non_leaf_aboveground_litterfall_tmp=dataset['non_leaf_aboveground_litterfall'][day,:]
    Belowground_litter_fall_tmp=dataset['Belowground_litter_fall'][day,:]
    ...
    data = np.column_stack((iveg,index_lat,index_lon,Leaf_fall_tmp,non_leaf_aboveground_litterfall_tmp,Belowground_litter_fall_tmp,
    ...
    print("column_stack")
    for (iveg_index, y, x, leaf_value,noleaf_value,belowground_value,
        ...
        Leaf_fall_new[day,iveg_index.astype(int)-1, y.astype(int), x.astype(int)] = leaf_value
        non_leaf_aboveground_litterfall_new[day,iveg_index.astype(int)-1, y.astype(int), x.astype(int)] = noleaf_value
        Belowground_litter_fall_new[day,iveg_index.astype(int)-1, y.astype(int), x.astype(int)] = belowground_value
...
ds['Leaf_fall']=Leaf_fall_new
ds['non_leaf_aboveground_litterfall']=non_leaf_aboveground_litterfall_new
ds['Belowground_litter_fall']=Belowground_litter_fall_new 
