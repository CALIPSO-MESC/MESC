...
for i in range(1,8):
    print(i)
    Clay_dataset=xr.open_dataset("/data1/zxy/SOC_data_calibration/HWSD/HWSD_Clay_0.5d_D"+str(i)+".nc", decode_times=False)
    Clay=Clay_dataset['Band1'][:]
    Clay = Clay.fillna(-99999)
    Clay = Clay.isel(lat=slice(None, None, -1))
    Clay_fraction[i-1,:]=Clay.values
    
    Silt_dataset=xr.open_dataset("/data1/zxy/SOC_data_calibration/HWSD/HWSD_Silt_0.5d_D"+str(i)+".nc", decode_times=False)
    Silt=Silt_dataset['Band1'][:]
    Silt = Silt.fillna(-99999)
    Silt = Silt.isel(lat=slice(None, None, -1))
    Silt_fraction[i-1,:]=Silt.values
    
    ph_dataset=xr.open_dataset("/data1/zxy/SOC_data_calibration/HWSD/HWSD_ph_0.5d_D"+str(i)+".nc", decode_times=False)
    ph=ph_dataset['Band1'][:]
    ph = ph.fillna(-99999)
    ph = ph.isel(lat=slice(None, None, -1))
    ph_data[i-1,:]=ph.values    

    gravel_dataset=xr.open_dataset("/data1/zxy/SOC_data_calibration/HWSD/HWSD_coarse_0.5d_D"+str(i)+".nc", decode_times=False)
    gravel=gravel_dataset['Band1'][:]
    gravel = gravel.fillna(-99999)
    gravel = gravel.isel(lat=slice(None, None, -1))
...
for i in range(0,5):
    print(i)
    Ald_dataset=xr.open_dataset("/data1/zxy/SOC_data_calibration/ren-gcb2024/reactive-minerals/Ald_"+str(i*20)+"_"+str(i*20+20)+".nc", decode_times=False)
    Ald=Ald_dataset['Band1'][:]
    Ald = Ald.fillna(-99999)
    Ald = Ald.isel(lat=slice(None, None, -1))
    Ald_data[i-1,:]=Ald.values
    
    Alo_dataset=xr.open_dataset("/data1/zxy/SOC_data_calibration/ren-gcb2024/reactive-minerals/Alo_"+str(i*20)+"_"+str(i*20+20)+".nc", decode_times=False)
    Alo=Alo_dataset['Band1'][:]
    Alo = Alo.fillna(-99999)
    Alo = Alo.isel(lat=slice(None, None, -1))
    Alo_data[i-1,:]=Alo.values
    
    Fed_dataset=xr.open_dataset("/data1/zxy/SOC_data_calibration/ren-gcb2024/reactive-minerals/Fed_"+str(i*20)+"_"+str(i*20+20)+".nc", decode_times=False)
    Fed=Fed_dataset['Band1'][:]
    Fed = Fed.fillna(-99999)
    Fed = Fed.isel(lat=slice(None, None, -1))
    Fed_data[i-1,:]=Fed.values
    
    Feo_dataset=xr.open_dataset("/data1/zxy/SOC_data_calibration/ren-gcb2024/reactive-minerals/Feo_"+str(i*20)+"_"+str(i*20+20)+".nc", decode_times=False)
    Feo=Feo_dataset['Band1'][:]
    Feo = Feo.fillna(-99999)
    Feo = Feo.isel(lat=slice(None, None, -1))
...
ds_new.to_netcdf('/data1/zxy/SOC_data_calibration/ORCHIDEE_data/hwsd-soil-orchidee.nc',encoding={
